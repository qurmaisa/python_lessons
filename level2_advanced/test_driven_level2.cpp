 ...
 *
 * 💡 ПОДСКАЗКИ:
 * - Используйте isalpha(c) для проверки, является ли символ буквой
 * - Используйте isupper(c) / islower(c) для определения регистра
 * - Используйте оператор % 26 для циклического сдвига
 * - Обрабатывайте заглавные и строчные буквы отдельно
 * - Не-буквы оставляйте без изменений
 */
 ...
 *
 * 💡 ПОДСКАЗКИ:
 * - Создайте вектор булевых значений размером n + 1
 * - Отметьте 0 и 1 как не простые
 * - Для каждого числа от 2 до √n:
 *   - Если оно простое, отметьте все его кратные как не простые
 *   - Начинайте с i² для оптимизации
 * - Соберите все простые числа в результирующий вектор
 */
